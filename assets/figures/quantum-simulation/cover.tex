...
\begin{tikzpicture}[x=1cm,y=1cm,line cap=round,line join=round,font=\sffamily]
  ...
  \node[anchor=west,text=mist,font=\bfseries\Large] at (1.15,7.65) {Statevector gate application is an indexing problem};
  \node[anchor=west,text=muted,font=\small] at (1.15,7.15) {Pair amplitudes whose basis indices differ only at the target bit.};
  ...
  \node[anchor=west,text=muted,font=\small] at (1.15,1.30) {$a_0=\operatorname{insert0}(i,q)$};
  \node[anchor=west,text=muted,font=\small] at (6.05,1.30) {$a_1=a_0\,\vert\,(1\ll q)$};
  \node[anchor=east,text=mist,font=\bfseries] at (14.85,1.30) {$2^{N-1}$ disjoint pairs};
\end{tikzpicture}
\end{document}
